...
class CBXBPToolbar_Public {
	...
	public function global_bp_toolbar($template_pack = 'legacy'){
		...
		if($show_toolbar){
			echo '<'.$nav_wrapper.' class="'.apply_filters('cbxbptoolbar_nav_wrapper', 'bp-navs cbxbptoolbar-navs').'">';
				echo '<ul class="'.apply_filters('cbxbptoolbar_nav_ul', 'cbxbptoolbar-navs-ul').'">';
				 ...
				 //echo '<li><a href="#">'.esc_html__('Blogs', '').'</a></li>';
				echo '</ul>';
			echo '</'.$nav_wrapper.'>';
		}
	...
	private function isMenuSelected($page_id){
		...
		if(is_page($page_id)) return 'selected';
